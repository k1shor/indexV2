...
export const getAllUsers = (token, search = "", page = 1, limit = 10) => {
  return fetch(
    `${API}/user/getallusers?search=${search}&page=${page}&limit=${limit}`,
    {
      headers: {
        Authorization: token,
      },
...
export const authenticate = (data) => {
  if (typeof window !== "undefined") {
    ...
    // Notify Header and other components that auth has changed
...
export const isAuthenticated = () => {
  ...
  try {
    return JSON.parse(stored).token || false;
  } catch {
...
// ✅ CHECK IF LOGGED-IN USER IS AN ADMIN (role >= 1)
...
// NOTE: Now served from TeamMemberRoutes — only returns admin-approved members
